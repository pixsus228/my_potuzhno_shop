...
class CartService:
    ...
    def add(self, product_id, quantity=1):
        ...
        if self.user:
            ...
            pid = str(product_id)
            current_qty = (
                self.cart_session.get(pid, {}).get("quantity", 0)
                if isinstance(self.cart_session.get(pid), dict)
                else self.cart_session.get(pid, 0)
            )
    ...
    def increase(self, product_id):
        if self.user:
            ...
            if pid in self.cart_session:
                current_qty = (
                    self.cart_session[pid].get("quantity", 1)
                    if isinstance(self.cart_session[pid], dict)
                    else self.cart_session[pid]
                )
        ...
        if self.user:
            ...
            if cart:
                for item in cart.items.select_related("product").all():
                    ...
                    img_url = (
                        p.image.url if hasattr(p, "image") and p.image else ""
                    )
                    cart_data[str(p.id)] = {
                        "id": p.id,
                        "name": p.name,
            ...
            pids = [
                int(pid) for pid in self.cart_session if str(pid).isdigit()
            ]
