...
class BrandProductListView(HomeView):
    def get_queryset(self):
        return (
            super().get_queryset().filter(brand__slug=self.kwargs.get("slug"))
        )
...
def toggle_favourite(request, slug):
    ...
    try:
        ...
        if product in wishlist.products.all():
            ...
            messages.warning(
                request, f"Товар {product.name} видалено з обраного."
            )
        else:
            wishlist.products.add(product)
            messages.success(
                request, f"Товар {product.name} додано до обраного."
            )
    except ImportError:
...
def review_delete(request, pk=None):
    if pk:
        ...
        if request.user.is_authenticated and (
            request.user == review.user or request.user.is_staff
        ):
            review.delete()
