...
class PotuzhnoShopUltimate100Tests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            ...
            email="test100@potuzhno.com",
        )
        self.profile, _ = Profile.objects.get_or_create(user=self.user)

        self.staff_user = User.objects.create_superuser(
            username="admin100", password="password123"
        )
    ...
    def test_001_category_str(self):
        self.assertEqual(str(self.category), "Худі")

    def test_002_brand_str(self):
        self.assertEqual(str(self.brand), "ПОТУЖНО")

    def test_003_size_str(self):
        self.assertEqual(str(self.size_l), "L")

    def test_004_product_str(self):
        self.assertEqual(str(self.product), "Потужне Худі Test")

    def test_005_profile_str(self):
        self.assertEqual(str(self.user.profile), "testuser100")

    def test_006_category_creation(self):
        self.assertTrue(Category.objects.filter(slug="hoodies").exists())

    def test_007_brand_creation(self):
        self.assertTrue(Brand.objects.filter(slug="potuzhno").exists())
    ...
    def test_010_product_sizes_count(self):
        self.assertEqual(self.product.sizes.count(), 2)

    # 11-20: Менеджери та QuerySet
    def test_011_queryset_active(self):
        self.assertIn(self.product, Product.objects.active())
    ...
    def test_015_category_ordering(self):
        self.assertEqual(str(Category._meta.ordering), "['-created_at']")

    def test_016_product_ordering(self):
        self.assertEqual(str(Product._meta.ordering), "['-created_at']")
    ...
    def test_018_product_audience_default(self):
        self.assertEqual(self.product.audience, "unisex")

    def test_019_product_featured_default(self):
        p3 = Product.objects.create(
            name="Звичайний", slug="regular", category=self.category, price=100
        )
        self.assertFalse(p3.is_featured)

    def test_020_product_price_type(self):
        self.assertIsInstance(self.product.price, Decimal)

    # 21-35: Відгуки (Reviews)
    def test_021_review_creation(self):
        Review.objects.create(
            user=self.user, product=self.product, rating=5, text="Топ!"
        )
    ...
    def test_023_review_uniqueness_user_product(self):
        Review.objects.create(
            user=self.user, product=self.product, rating=5, text="Перший"
        )
        with self.assertRaises(Exception):
            Review.objects.create(
                user=self.user, product=self.product, rating=3, text="Другий"
            )

    def test_024_review_rating_choices(self):
        r = Review.objects.create(
            user=self.user, product=self.product, rating=1
        )
        self.assertEqual(r.rating, 1)

    def test_025_review_max_length_text(self):
        r = Review.objects.create(
            user=self.user, product=self.product, rating=4, text="А" * 1000
        )
        self.assertEqual(len(r.text), 1000)

    def test_026_review_ordering(self):
        self.assertEqual(str(Review._meta.ordering), "['-created_at']")
    ...
    def test_031_favourites_initial_empty(self):
    ...
    def test_035_favourites_multiple_products(self):
        p2 = Product.objects.create(
            name="Товар 2", slug="t2", category=self.category, price=100
        )
    ...
    def test_038_profile_one_to_one_user(self):
    ...
    def test_048_cart_item_multiple_products(self):
        cart = Cart.objects.create(user=self.user)
        p2 = Product.objects.create(
            name="P2", slug="p2", category=self.category, price=200
        )
    ...
    def test_056_order_ordering(self):
        self.assertEqual(str(Order._meta.ordering), "['-created_at']")
    ...
    def test_059_order_items_related_name(self):
        ...
        OrderItem.objects.create(
            order=order, product=self.product, price=100, quantity=1
        )
    ...
    def test_062_product_form_invalid_missing_name(self):
        f = ProductForm(
            data={"slug": "t", "category": self.category.id, "price": "100"}
        )
    ...
    def test_071_home_view_status(self):
        self.assertEqual(self.client.get("/").status_code, 200)

    def test_072_product_list_view_status(self):
        self.assertEqual(self.client.get("/products/").status_code, 200)

    def test_073_product_detail_view_status(self):
        self.assertEqual(
            self.client.get(f"/products/{self.product.slug}/").status_code, 200
        )

    def test_074_contact_view_status(self):
        self.assertEqual(self.client.get("/contact/").status_code, 200)

    def test_075_cart_detail_view_status(self):
        self.assertEqual(self.client.get("/cart/").status_code, 200)

    def test_076_profile_unauthenticated_redirect(self):
        self.assertEqual(
            self.client.get("/accounts/profile/").status_code, 302
        )

    def test_077_profile_authenticated_success(self):
        self.client.login(username="testuser100", password="password123")
        self.assertEqual(
            self.client.get("/accounts/profile/").status_code, 200
        )

    def test_078_profile_edit_authenticated_success(self):
        self.client.login(username="testuser100", password="password123")
        self.assertEqual(
            self.client.get("/accounts/profile/edit/").status_code, 200
        )
    ...
    def test_080_login_page_status(self):
        self.assertEqual(self.client.get("/accounts/login/").status_code, 200)

    def test_081_register_page_status(self):
        self.assertEqual(
            self.client.get("/accounts/register/").status_code, 200
        )

    def test_082_password_change_redirect(self):
        self.assertEqual(
            self.client.get("/accounts/password-change/").status_code, 302
        )
    ...
    def test_085_admin_login_redirect(self):
    ...
    def test_088_product_update_view_staff(self):
        ...
        self.assertEqual(
            self.client.get(
                f"/products/{self.product.slug}/edit/"
            ).status_code,
            200,
        )
    ...
    def test_089_product_delete_view_staff(self):
        ...
        self.assertEqual(
            self.client.get(
                f"/products/{self.product.slug}/delete/"
            ).status_code,
            200,
        )
