...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        cats = {
            s: Category.objects.get_or_create(slug=s, defaults={"name": n})[0]
            for s, n in CATEGORIES.items()
        }
        brands = {
            s: Brand.objects.get_or_create(slug=s, defaults={"name": n})[0]
            for s, n in BRANDS.items()
        }
        sizes = {
            lbl: Size.objects.get_or_create(name=lbl)[0]
            for lbl in SIZES_LETTER + SIZES_SHOE
        }
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Успішно створено нових товарів: {created} (усього: {Product.objects.count()})."
            )
        )
